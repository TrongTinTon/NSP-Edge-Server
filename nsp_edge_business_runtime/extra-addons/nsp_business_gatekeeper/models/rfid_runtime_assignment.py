...
class NspRfidRuntimeAssignment(models.Model):
    ...
    user_id = fields.Many2one("nsp.user", ondelete="cascade", index=True)
    vehicle_id = fields.Many2one("nsp.vehicle", ondelete="cascade", index=True)
    ...
    def init(self):
        self.env.cr.execute(
            """
            CREATE UNIQUE INDEX IF NOT EXISTS nsp_rfid_runtime_user_uniq
                ON nsp_rfid_runtime_assignment (user_id)
             WHERE user_id IS NOT NULL
            """
        )
        self.env.cr.execute(
            """
            CREATE UNIQUE INDEX IF NOT EXISTS nsp_rfid_runtime_vehicle_uniq
                ON nsp_rfid_runtime_assignment (vehicle_id)
