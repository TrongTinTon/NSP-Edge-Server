...
class NspEdgeServer(models.Model):
    ...
    name = fields.Char(
        string="Edge Server Name",
        required=True,
        default="NSP Edge Server",
    )
    ...
    _sql_constraints = [
        (
            "edge_server_code_unique",
            "unique(edge_server_code)",
            "Edge Server Code must be unique.",
        ),
    ]
    ...
    def create(self, vals_list):
        ...
        for source in vals_list:
            ...
            values["name"] = str(
                values.get("name") or values["edge_server_code"]
            ).strip()
...
class NspController(models.Model):
    ...
    _sql_constraints = [
        (
            "controller_id_unique",
            "unique(controller_id)",
            "Controller Code must be unique.",
        ),
    ]
    ...
    def create(self, vals_list):
        ...
        for source in vals_list:
            ...
            values["controller_name"] = str(
                values.get("controller_name") or values["controller_id"]
            ).strip()
            prepared.append(values)
        return super().create(prepared)
    ...
    def cron_mark_offline_controllers(self):
        parameter = self.env["ir.config_parameter"].sudo().get_param(
            "nsp_business_gatekeeper.controller_heartbeat_timeout_sec",
            "120",
        )
        try:
            timeout = int(parameter or "120")
        ...
        cutoff = fields.Datetime.now() - timedelta(seconds=max(timeout, 30))
        # This system cron owns Controller liveness for the complete Edge scope.
