...
class NspParkingAreaCloudActions(models.Model):
    _inherit = "nsp.parking.area"

    def action_set_operational(self):
        areas = self._check_parking_action_access("write")
        for area in areas:
            area._publish("operational")
        return True

    def action_set_maintenance(self):
        areas = self._check_parking_action_access("write")
        for area in areas:
            area._publish("maintenance")
        return True

    def action_set_blocked(self):
        areas = self._check_parking_action_access("write")
        for area in areas:
            area._publish("blocked")
        return True

    def action_reset_to_draft(self):
        # The last immutable published payload remains active on Edge while Cloud is edited.
        areas = self._check_parking_action_access("write")
