...
class NspMeasurementSession(models.Model):
    ...
    def write(self, vals):
        values = dict(vals)
        lifecycle_fields = {"status", "revision", "started_at", "ended_at"}
        if lifecycle_fields.intersection(values) and not self.env.context.get("measurement_sync"):
            raise ValidationError(_(
                "Use Release, Revise, Stop or Cancel to change Lane Calibration lifecycle."
            ))
        if "measurement_code" in values and not self.env.context.get("measurement_sync"):
            raise ValidationError(_("Lane Calibration Code cannot be changed after creation."))
        if "target_line_ids" in values:
            if not self.env.context.get("measurement_sync") and self.filtered(lambda rec: rec.status != "draft"):
    ...
    def unlink(self):
        if not self.env.context.get("measurement_sync"):
            protected = self.filtered(lambda rec: rec.status != "draft")
            if protected:
                raise ValidationError(_("Only Draft Lane Calibration can be deleted."))
        return super().unlink()
    ...
    def _validate_measurement_scope(self):
        # Draft allows missing child levels, but every existing node already obeys
        # Server -> Controller -> Reader hierarchy. Release checks branch completeness.
        self._validate_calibration_tag_scope()
...
class NspMeasurementTargetLine(models.Model):
    ...
    @api.model
    def _ensure_editable_session(self, session):
        if session and not self.env.context.get("measurement_sync") and session.status != "draft":
            raise ValidationError(_("Calibration Tag can be changed only while Lane Calibration is Draft."))
    ...
    def create(self, vals_list):
        ...
        for source in vals_list:
            values = dict(source)
            session = self.env["nsp.measurement.session"].browse(int(values.get("session_id") or 0)).exists()
            self._ensure_editable_session(session)
    ...
    def write(self, vals):
        for line in self:
            self._ensure_editable_session(line.session_id)
        values = dict(vals)
        if "tid" in values:
            values["tid"] = self._normalize_tid(values.get("tid"))
            if not values["tid"]:
    ...
    def unlink(self):
        ...
        result = super().unlink()
        sessions._validate_calibration_tag_scope()
        return result
