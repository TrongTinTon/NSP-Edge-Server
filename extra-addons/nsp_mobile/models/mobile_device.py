...
class NspMobileDevice(models.Model):
    ...
    push_provider = fields.Selection([
        ('none', 'None'), ('fcm', 'Firebase Cloud Messaging'),
        ('apns', 'Apple Push Notification Service'), ('custom', 'Custom'),
    ...
    def _compute_display_name(self):
        for rec in self:
            label = rec.device_name or dict(rec._fields['platform'].selection).get(rec.platform) or _('Device')
    ...
    def touch(self, sync=False):
        vals = {'last_seen_at': fields.Datetime.now()}
        if sync:
            vals['last_sync_at'] = vals['last_seen_at']
