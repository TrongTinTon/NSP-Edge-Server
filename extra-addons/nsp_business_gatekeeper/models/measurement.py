...
class NspMeasurementSession(models.Model):
    ...
    def action_view_events(self):
        self.ensure_one()
        action = self.env.ref("nsp_business_gatekeeper.action_nsp_measurement_event").read()[0]
    ...
    def get_live_snapshot(self, session_id, last_event_id=0, limit=500):
        ...
        return {
            ...
            "edge_server_code": self.env["nsp.sync.auth"].sudo().search([], limit=1).edge_server_code if "nsp.sync.auth" in self.env.registry.models else "",
    ...
    def cron_cleanup_expired_measurements(self):
        ...
        events.unlink()
        # Measurement Sessions are Edge runtime cache, not master data. Once a
        # terminal session has no retained observations, remove the empty shell.
